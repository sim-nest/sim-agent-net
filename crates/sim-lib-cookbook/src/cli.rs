//! CLI entrypoint claim for the cookbook lib.
...
const COOKBOOK_CLI_VERB: &str = "cookbook";

fn cli_main_symbol() -> Symbol {
    Symbol::qualified("cli", "main/cookbook")
}

pub(crate) fn cookbook_cli_exports() -> Vec<Export> {
    vec![Export::Function {
...
pub(crate) fn register_cookbook_cli(cx: &mut LoadCx, linker: &mut Linker<'_>) -> Result<()> {
    linker.function_value(
        cli_main_symbol(),
        cx.factory().opaque(Arc::new(CookbookCliEntrypoint))?,
    )?;
    Ok(())
}

#[derive(Clone)]
struct CookbookCliEntrypoint;

impl Object for CookbookCliEntrypoint {
    fn display(&self, _cx: &mut Cx) -> Result<String> {
        Ok("#<function cli/main/cookbook>".to_owned())
    }
...
impl ObjectCompat for CookbookCliEntrypoint {
...
impl Callable for CookbookCliEntrypoint {
    fn call(&self, cx: &mut Cx, args: Args) -> Result<Value> {
        verify_cli_envelope(cx, &args, COOKBOOK_CLI_VERB)?;
